...
// https://stackoverflow.com/a/66124330
export const interpolateColor = (from: string, to: string, percentage: number): string => {
  if (from === to) {
    return from;
  }
  ...
  const red = Math.round(fromColorRgb[0] * q + toColorRgb[0] * percentage);
  const green = Math.round(fromColorRgb[1] * q + toColorRgb[1] * percentage);
  const blue = Math.round(fromColorRgb[2] * q + toColorRgb[2] * percentage);
